...
const Result: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchResultData = async () => {
      try {
        ...
        const questionsData = (questionsDoc.data().question_question || []) as Question[];
        ...
        const answerKeysData = (answerKeysDoc.data().answer_answer || []) as AnswerKey[];
        ...
        if (studentInfo && studentInfo.name && studentInfo.id) {
          ...
          const submissionData = submissionsSnapshot.docs[0].data() as Submission;
          ...
          questionsData.forEach((question: Question, index: number) => {
            ...
            const correctAnswer = answerKeysData[index] || { answer: [] };
            
            if (question.type === 'multiple-choice') {
              // For multiple choice, compare arrays
              if (Array.isArray(studentAnswer) && Array.isArray(correctAnswer.answer)) {
                const isCorrect = 
                  studentAnswer.length === correctAnswer.answer.length &&
                  studentAnswer.every((answer: any) => correctAnswer.answer.includes(answer));
                
                if (isCorrect) correctCount++;
              }
            } else {
              // For short answer questions, exact match (can be improved with fuzzy matching)
              if (String(studentAnswer).trim().toLowerCase() === String(correctAnswer.answer).trim().toLowerCase()) {
                correctCount++;
              }
            }
          });
  ...
  const formatTime = (startTime: any, endTime: any): string => {
    ...
    <div className="result-container">
      ...
      <div className="questions-section">
        ...
        {questions.map((question: Question, index: number) => {
          ...
          const correctAnswer = answerKeys[index] || { answer: [] };
          let isCorrect = false;
          ...
          if (question.type === 'multiple-choice') {
            ...
            isCorrect = String(studentAnswer).trim().toLowerCase() === String(correctAnswer.answer).trim().toLowerCase();
          }
              ...
              {question.type === 'multiple-choice' ? (
                <div className="options">
                  {question.options?.map((option: string, optIndex: number) => {
                    ...
                    const isCorrectOption = Array.isArray(correctAnswer.answer) && correctAnswer.answer.includes(optIndex);
                ...
                <div className="short-answer-result">
                  ...
                  <div className="correct-answer">
                    <strong>Đáp án đúng:</strong> {correctAnswer.answer || 'N/A'}
                  </div>
                </div>
              )}
            </div>
