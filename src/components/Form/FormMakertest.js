...
function FormMaker() {
  ...
  const [list, setList] = useState([
    { id: uuid(), question: "", answer: null, questionType: "mcq" },
  ]);
  ...
  function handleAddQuest() {
    ...
    setList([...list, { id: uuid(), question: "", answer: null, questionType: "mcq" }]);
  }
  ...
  async function onSubmit() {
    console.log(list, optionList, trueFalseOptions);
    let val = validator();
    if (val) {
  ...
  function renderOptions(index) {
    const questionType = list[index].questionType;
    if (questionType === "mcq") {
      return optionList[index].map((soloOption, ind) => (
      ...
      return (
        <li>
          ...
          /> */}
          {trueFalseOptions.map((option, ind) => (
            <div key={option.id}>
              <input
                type="text"
                className="optionBox faintShadow"
                ...
                onChange={(event) => {
                  ...
                  newOptions[ind].opinion = event.target.value;
              ...
              <select
                onChange={(event) => {
                  const newOptions = [...trueFalseOptions];
                  newOptions[ind].answer = event.target.value;
                  setTrueFalseOptions(newOptions);
                }}
                className="optionBox faintShadow"
              >
          ...
          <button onClick={() => handleAddOpinion(index)} className="sub_btn_actual hov">
            Thêm Ý Kiến
          </button>
        </li>
      );
  ...
  function handleAddOpinion(index) {
    setTrueFalseOptions([...trueFalseOptions, { id: uuid(), opinion: "", answer: null }]);
  }
      ...
      {list.map((soloList, index) => (
        <div key={soloList.id} id="questionnaire">
          <ul>
            <li>
              <select
                value={soloList.questionType}
                onChange={(event) => questChangeHandler(event, index, "questionType")}
              >
