...
Chỉ trả về text theo đúng định dạng trên, không giải thích gì thêm.`;
  ...
  async function extractQuestions(file, setRawText, setQuizTitle, quizTitle) {
    if (!file) {
  ...
  async function matrixQuestion(file, setRawText) {
    if (!file) {
      warning("Vui lòng chọn file trước.", 4000);
      return;
    }
...
Chỉ trả về text theo đúng định dạng trên, không có giải thích hay văn bản thừa nào khác.`;
  ...
  async function createQuestions(mcqRef, trueFalseRef, shortAnswerRef, list, optionList, setRawText) {
...
Chỉ trả về text theo đúng định dạng trên, không có giải thích hay văn bản thừa nào khác.`;
      ...
      if (ketQua) {
        ...
        const totalCount = parseInt(mcqCount) + parseInt(trueFalseCount) + parseInt(shortAnswerCount);
  ...
  function serializeCurrentQuestions(list, optionList) {
