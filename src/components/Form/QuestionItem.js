...
function QuestionItem({
  ...
  const [showCommonFormulas, setShowCommonFormulas] = useState(false);
  ...
  const handleInsertMath = () => {
    ...
    if (targetField === 'question') {
      ...
      onQuestionChange({ target: { value: newQuestion } }, index, "question");
    } else if (targetField === 'option' && targetOptionIndex !== null) {
      const newOption = options[targetOptionIndex].option + formula;
      onOptionChange({ target: { value: newOption } }, index, targetOptionIndex);
    } else if (targetField === 'answer' && question.type === 'shortanswer') {
      const newAnswer = question.answer + formula;
      onQuestionChange({ target: { value: newAnswer } }, index, "answer");
    }
    setMathInput('');
  ...
  const openMathModal = (field, optionIndex = null) => {
        ...
        <li className="pt-3">
          <div className="flex flex-wrap gap-3">
            ...
            {/* <button
              ...
              onClick={() => openMathModal('question')}
            >
              <FaSquareRootAlt className="mr-2 text-lg"/>
              Chèn công thức
            </button> */}
            <ImageUploader
              index={index}
