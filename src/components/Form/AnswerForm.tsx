...
// @ts-ignore - react-beforeunload may not have types
import { useBeforeunload } from 'react-beforeunload';
import db from '../../services/firebaseConfig';
// @ts-ignore - CountdownTimer may not exist or may not have types
import CountdownTimer from '../Timer/CountdownTimer';
import './AnswerForm.css';
// @ts-ignore - react-bootstrap may not have types
...
const AnswerForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchExamDetails = async () => {
      try {
        if (!id) {
          throw new Error("Không tìm thấy mã phiên thi.");
        }
  ...
  const handleAnswerChange = (index: number, value: string, type: string) => {
    const newAnswers = [...answers];
    
    if (type === 'multiple-choice') {
      // For multiple choice questions
      const optionIndex = parseInt(value);
      const currentAnswers = [...(newAnswers[index] as number[])];
                  ...
                  {question.options?.map((option, optIndex) => (
                    <div key={optIndex} className="form-check">
                      <input
                        ...
                        value={optIndex}
                        checked={(answers[index] as number[])?.includes(optIndex) || false}
