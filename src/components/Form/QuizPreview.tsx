...
const QuizPreview: React.FC<QuizPreviewProps> = ({ quizTitle, questions, options, onAnswerChange }) => {
  ...
  return (
    <div className="quiz-preview-container p-6 bg-white rounded-lg shadow-md h-full overflow-y-auto">
      <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 border-b pb-4">
        {quizTitle || 'Xem trước đề thi'}
      </h2>
      <div className="space-y-6" style={{ fontWeight: 'normal' }}>
        {questions.map((question, index) => {
          ...
          if (question.type === 'textblock') {
            return (
              <div key={question.id || index} className="p-4 border-2 border-blue-200 rounded-lg bg-blue-50">
                <div className="prose max-w-none text-gray-800 whitespace-pre-wrap" style={{ fontWeight: 'normal' }}>
          ...
          <div key={question.id || index} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
            ...
            <div className="prose max-w-none text-gray-800 mb-3" style={{ fontWeight: 'normal' }}>
                ...
                {options[index] && options[index].length > 0 ? (
                  options[index].map((opt, optIndex) => (
                    ...
                    >
                      ...
                      <div className="prose-sm max-w-none flex-1" style={{ fontWeight: 'normal' }}>
                      ...
                      {question.answer === optIndex && (
                        <span className="ml-2 text-green-600 text-sm">✓ Đúng</span>
                      )}
                    </div>
                  ))
                ) : (
                ...
                {options[index] && options[index].length > 0 ? (
                  options[index].map((opt, optIndex) => (
                    ...
                    >
                      ...
                      <div className="flex-1 prose-sm max-w-none" style={{ fontWeight: 'normal' }}>
                        {opt.option && opt.option.trim() ? (
                          <MathJax dynamic inline>{opt.option}</MathJax>
                        ) : (
            ...
            {question.type === 'shortanswer' && (
              <div className="mt-4 pl-4">
                {question.answer && (Array.isArray(question.answer) ? question.answer.length > 0 : (typeof question.answer === 'string' && question.answer.trim())) ? (
                  <div>
                    <p className="text-sm text-gray-700 mb-2">Đáp án hợp lệ:</p>
                    <div className="flex flex-wrap gap-2">
                      {(Array.isArray(question.answer) ? question.answer : [question.answer]).map((ans, ansIndex) => (
                        <span 
                          key={ansIndex}
                          className="px-3 py-1.5 bg-green-100 text-green-800 rounded-md border border-green-300 text-sm"
                        >
                          {typeof ans === 'string' ? ans.trim() : String(ans)}
                        </span>
                      ))}
                    </div>
                  </div>
