...
import { collection, getDocs, addDoc, doc, updateDoc, deleteDoc, setDoc, serverTimestamp } from "firebase/firestore/lite";
...
const QuestionBank: React.FC = () => {
  ...
  const refreshAnalysisJobs = async () => {
    try {
      if (auth.currentUser) {
        ...
        const jobs: AnalysisJob[] = snapshot.docs.map((docSnap) => ({
          id: docSnap.id,
          ...docSnap.data()
        } as AnalysisJob)).sort((a, b) => getTimestampValue(b.createdAt) - getTimestampValue(a.createdAt));
  ...
  const openEditModal = (bank: QuestionBank, e: MouseEvent<HTMLButtonElement>) => {
  ...
  const openDeleteModal = (bank: QuestionBank, e: MouseEvent<HTMLButtonElement>) => {
  ...
  const openJobPreview = (job: AnalysisJob) => {
    if (!job?.rawResponse) {
  ...
  const handleConfirmAnalysis = async () => {
    ...
    try {
      ...
      alert("Có lỗi xảy ra khi lưu ngân hàng câu hỏi: " + (error.message || String(error)));
    }
  };
  ...
  const handleEditSubmit = async () => {
    if (!editingBank || !auth.currentUser) return;
  ...
  const handleDeleteConfirm = async () => {
    if (!deletingBank || !auth.currentUser) return;
  ...
  const processFile = async () => {
    ...
    try {
      ...
      const resultText = typeof analysisResult === 'string' 
        ? analysisResult 
        : (analysisResult as any).text || analysisResult;
      const resultSnapshot = typeof analysisResult === 'string' 
        ? undefined 
        : (analysisResult as any).snapshot;
      ...
      setJobError(error.message || String(error));
      if (jobDocRef) {
        try {
          await updateDoc(jobDocRef, {
            status: "failed",
  ...
  const viewQuestionBank = (bankId: string) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-3xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4 mb-6">
              {analysisPreview.cleanResult.chapters?.map((chapter: any, chapterIndex: number) => (
