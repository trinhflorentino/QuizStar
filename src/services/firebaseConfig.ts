...
var realtimeDb: Database = getDatabase(app);
    ...
    getAuth().onAuthStateChanged((user) => {
      ...
      if (user) {
        for (let i = 0; i < some.length; i++) {
          (some[i] as HTMLElement).style.display = "unset";
        }
        for (let i = 0; i < signElements.length; i++) {
          (signElements[i] as HTMLElement).style.display = "none";
        }
        const name = document.querySelector("#displayName");
        if(name) {
          name.innerHTML = user.displayName || "";
        }
        const avatarElement = document.querySelector("#profileAvatar") as HTMLImageElement;
        if (avatarElement) {
          avatarElement.src = user?.photoURL || require("../images/profile.jpg");
        }
      } else {
        for (let i = 0; i < some.length; i++) {
          (some[i] as HTMLElement).style.display = "none";
        }
        for (let i = 0; i < signElements.length; i++) {
          (signElements[i] as HTMLElement).style.display = "block";
        }
      }
    });
  }, [authState]);
...
// export storage;
// export default db;
export var storage: FirebaseStorage = getStorage(app);
export var auth: Auth = getAuth(app);
export var realtimeDatabase: Database = realtimeDb;
