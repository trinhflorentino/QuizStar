...
function MatchNameBanner() {
    ...
    var refStatus = firebase.database().ref(matchid + "/Banner/Elements");
    refStatus.update(data);
    Notification("Đã bật giao diện tên trận đấu");
}
    ...
    snapshot.forEach(function (childSnapshot) {
        ...
        if (childData.requestStatus === 0) {
            ...
            if (childData.role === "Viewer") {
                joinText.textContent = childData.displayName + " yêu cầu xem trận đấu";
...
function updateRequestStatus(pushId, status) {
    ...
    switch (status) {
        case 1:
            Notification("Đã chấp nhận yêu cầu xem trận đấu");
            break;
        case 2:
            Notification("Đã từ chối yêu cầu xem trận đấu");
            break;
        case 3:
            Notification("Đã chặn xem trận đấu");
            break;
    }
}
