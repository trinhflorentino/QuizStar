...
auth.onAuthStateChanged(_0x4e5d07 => {
  ...
  successToast("Đang lấy thông tin phòng trò chơi tham gia gần nhất", 0xbb8, "top", 'right', true, false, '');
  ...
  _0x2d1a5b.onSnapshot(_0x385e43 => {
    ...
    _0x50adb4.on('value', _0x1e9208 => {
      ...
      if (!_0xa50be1) {
        failToast("Không tìm thấy phòng trò chơi hoặc thông tin không hợp lệ", 0xbb8, 'top', "right", true, false, '');
        document.getElementById('matchJoinCard').style.display = 'none';
        return;
      }
      ...
      document.getElementById("joinAnotherMatchTitle").innerHTML = "Tham gia phòng trò chơi khác";
    });
  }, _0x66db11 => {
...
matchJoinForm.addEventListener("submit", _0x272c56 => {
  ...
  _0x158ad1.on('value', _0x3138ba => {
    ...
    if (!_0x79d31) {
      failToast("Mã phòng trò chơi không tồn tại", 0xbb8, "top", "right", true, false, '');
      clearInputs();
      return;
    }
    if (_0x2e4b2e !== _0x79d31) {
      failToast("Mã phòng trò chơi không đúng", 0xbb8, 'top', 'right', true, false, '');
      clearInputs();
      return;
    }
    _0x2de1e0.set({
